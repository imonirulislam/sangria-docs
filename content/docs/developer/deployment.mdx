...
# Social sign-in (all optional). A provider is only offered on the sign-in screen
# when both of its vars are set, so set them in pairs or not at all. Register
# <SITE_URL>/api/auth/callback/<provider-id> with each provider.
...
bunx convex env set AUTH_GITLAB_ID <client id>
bunx convex env set AUTH_GITLAB_SECRET <client secret>
# Microsoft: TENANT_ID is optional and defaults to "common" (any Microsoft account);
# set your directory id to restrict sign-in to one organisation.
bunx convex env set AUTH_MICROSOFT_ENTRA_ID_ID <client id>
bunx convex env set AUTH_MICROSOFT_ENTRA_ID_SECRET <client secret>
bunx convex env set AUTH_MICROSOFT_ENTRA_ID_TENANT_ID <directory id>
# Apple's client secret is a JWT generated from a .p8 key and expires within six
# months, so it has to be regenerated on a schedule. Apple also rejects
# http/localhost redirects, so this one can't be tested in local dev.
bunx convex env set AUTH_APPLE_ID <services id>
bunx convex env set AUTH_APPLE_SECRET <generated JWT>
...
bunx convex env set FCM_SERVICE_ACCOUNT_JSON -- "$(cat firebase-service-account.json)"
```
...
- `NEXT_PUBLIC_VERCEL_ANALYTICS_ENABLED` — set to `true` only on the demo deploy to
  mount Vercel Analytics + Speed Insights; leave it unset (or `false`) in
  production. It's baked into the app image as a build arg, so it's decided
